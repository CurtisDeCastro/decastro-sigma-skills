...
> # ⚠ PARTIALLY STALE — do not author from this file's shapes
>
> Sigma changed the spec format on **2026-08-10** and this file (1598 lines) still
> teaches the old one in ~25 places. It has NOT been fully migrated. Specific
> corrections are marked inline and dated, but assume any *envelope, layout-tag, or
> element-shape* example here is pre-migration unless it says otherwise.
>
...
>    '{"workbookId":"<id>"}' --yaml`.
>
> The 30-second version of what changed: `document` envelope; `elements` FLAT (not
> under `pages`); tags `<Element>`/`<Container>`; theme at
> `document.settings.theme.overrides`; modals in `document.overlays[]`; agents in
> `document.agents`; an unplaced element is rejected.
>
> **What is still uniquely valuable here** and exists nowhere else: the aggregation
> patterns — multi-level `groupings` (`groupBy` + `calculations`), the legacy
> `{id}`-only groupings warning, `Rollup` vs `Lookup`, `DateLookback`,
> materialize-then-window, the summary-bar aggregate-then-categorize pattern,
> two-tier sourcing, and verify-via-generated-SQL. Those are format-independent and
> still correct. Extracting them into `aggregation-patterns.md` and gutting the rest
> is the outstanding task (Phase 2 of `docs/format-migration-plan.md`).

Canonical patterns for the workbook spec endpoints. These rules are the
...
- **KPI period-comparison configuration** (e.g. "vs prior month" vs "vs prior
  ...
  set it in the spec.
- **Pivot-table cell-color conditional formatting (heatmap visual)** — NUANCED,
  corrected 2026-08-28. Applying it **in the UI** is what breaks the GET-spec
  endpoint (500) for that workbook, at every version. But a heatmap authored
  **in the spec** works and round-trips cleanly: `conditionalFormats` entries of
  `type: "single"` with `condition: "formula"` and non-overlapping value bands.
  So: author heatmaps in code, never by clicking. See `examples/report-workbook-exemplar.json`
  for a full diverging-band implementation.
...
| Pivot table | `pivot-table` | `rowsBy: [{id}]`, `columnsBy: [{id}]`, `values: ["<col-id>", ...]` | **Only this exact shape — `rows`/`cols`/`values`-as-objects is rejected.** See "Pivot-table element shape." Heatmap cell-colour: author it in the SPEC (`conditionalFormats`, `condition:"formula"`, banded) — that round-trips. Applying it in the **UI** permanently breaks GET-spec for the workbook. Also supports `display.rowLayout`, `totals`, and `rowsBy[].sort.by`. |
...
**~~Still not supported (rejected): popovers, navigation elements, forms, page breaks,
value lists, repeated containers, single-row containers, Python elements.~~ — DELETED,
2026-08-13.** This list was wrong and self-contradicting. `repeated-container`,
`waterfall`, `navigation`, `page-break` and `gauge` are all documented as first-class by
the official `sigma-workbooks-cli` skill, and **our own
`sigma-workbook-styling/SKILL.md` teaches repeated containers.** Don't infer that a kind
is unsupported from a doc — probe it: `sigma api workbooks spec verify` names the exact
JSON path, and a masked `Invalid kind` almost always means a wrong field shape rather
than a missing feature (see above).

**Corrected 2026-07-30 (probed against `POST /v2/workbooks/spec/verify`):**
- `navigate` **works**: `{kind:"effect",effect:"navigate",target:{type:"page",page:"<pageId>"}}`.
  An unknown `target.type` errors with `unknown navigate target`, so `"page"` is a real member.
  No need to fall back to `open-url _self`.
- Action **sequences** are a valid step shape (`{kind:"sequence",sequenceId}`) — they just
  reference something the workbook must already define. Not "rejected".
- `select-tab`, `clear-control`, `refresh-element`, `update-rows` are all **recognized effect
  names** (they fail on payload shape, not on the name — unlike `export` /
  `send-notification`, which return `unknown action effect`). Shapes not yet worked out.
See the `agents` block in `../examples/exemplar-spec.json`. Also box plot, waterfall, sankey, funnel, gauge. Build
these in the UI.
...
```bash
# The CLI handles auth, host, and request shape.
sigma api workbooks elements query get \
  --params '{"workbookId":"<id>","elementId":"<element-id>"}' | jq -r '.sql'
```

(Don't confuse this with `scripts/api/query-element.sh` — that exports the
element's *rendered data* as CSV/JSON, not its generated SQL.)
...
## Column `format` — POSTS FINE (this section used to say otherwise)

**Corrected 2026-08-13.** This section previously told you to omit `format` from POST
bodies because "the required `kind` value is not in the public docs." The `kind` is
`"number"` or `"datetime"`, it POSTs fine, and the canned generator uses it on 64
columns. Set it from code; don't configure formatting in the UI after CREATE.

```jsonc
// currency
{"kind":"number","formatString":"$.3~s","currencySymbol":"$",
 "decimalSymbol":".","digitGroupingSymbol":",","digitGroupingSize":[3]}
{"kind":"number","formatString":".1%"}        // percent
{"kind":"number","formatString":"+,.1%"}      // signed percent, for deltas
{"kind":"datetime","formatString":"%b %Y"}    // datetime axis
```

What IS rejected is the shape the old text was probing with —
`{type:"number", format:"currency", currency:"USD"}` — which yields
`columns[M].format: Missing "kind" field`. That is a wrong-shape error, not an
unsupported field.
...
# Verified additions — report-replication build (2026-08-28)

Every item below cost at least one render to find, on a real build that reproduced a
customer's static risk report. Worked exemplar:
`examples/report-workbook-exemplar.json`. Procedure: `../SKILL.md` →
"Rebuilding an existing report".

## Page headers ARE code-representable

Supersedes any note calling them UI-only. Three coordinated parts:

```jsonc
"panels":   [{"id":"<id>", "type":"header", "title":"Header 1", "pages":["pg","util"]}],
"settings": {"navigation": {"pageHeader": "enabled"}}
```
plus a **`<Panel …>` tag as a sibling of `<Page>`** in the layout XML holding the
header's elements. It survives GET. The header renders on every page, which also makes
it a convenient always-visible test surface.

## Formula syntax

- Boolean conjunction is the **`and` keyword**. There is no `And()` function —
  `And(a, b)` is rejected with `Invalid formula`.
- Booleans compare bare: `[Col] = True`. `= "true"` → `Argument 2 invalid for function '='`.
- Element/column references split on `/`, so a **column name containing `/` breaks**:
  `[Vega $ / pt]` parses as element "Vega $", column "pt". Rename the column.

## Data-model-sourced elements (two-tier sourcing)

A workbook table reading a data model must qualify its passthrough columns with the
**data-model element's NAME**: `[Scenario PnL/P&L]`. A bare `[P&L]` passes spec/verify,
passes a linter, and round-trips through GET — then renders every downstream tile as
"Reference to errored column". The workbook table's own `name` must therefore differ
from the data-model element name. A table element with **no `columns`** is rejected
(`Invalid kind: "table"`).

## kpi-chart

- Fixed target: a constant-formula column + `comparisonColumn` + `comparison`
  `{"display":"delta","colorGood":…,"colorBad":…}` → `↑ 3,158,415 vs Limit -400,000`,
  green above / red below, automatically from the sign.
- `comparison.display`: `delta` | `absolute`. `percentDelta` rejected.
- `comparison.direction`: only `"none"` accepted; omit for default sign colouring.
- `description` renders as an **ⓘ tooltip**, not visible text.
- ⚠️ **`conditionalFormats` on a kpi-chart passes verify then CRASHES the renderer.**
- An unrecognised *field* on a kpi is masked as `Invalid kind: "kpi-chart"`.

## pivot-table

- `display: {"rowLayout": "separate-columns"}` renders multiple `rowsBy` as real
  side-by-side columns instead of one indented stack.
- `totals: {"showGrandTotals":"hidden","showSubtotals":"when-collapsed"}`.
- Sort rows by a value column with **`by`**, not `columnId`:
  `{"id":"x","sort":{"by":"<value-col>","direction":"descending"}}`. The `columnId` form
  verifies and then silently sorts the dimension alphabetically — confirm sorts by
  rendering. Tables instead use a top-level
  `"sort":[{"columnId":…,"direction":…,"nulls":"last"}]`.

## Text elements, HTML and `{{ }}`

- Bodies accept HTML with Sigma's typography classes: `<p class="p-large">`,
  `<p class="p-small">`; markdown nests inside.
- Coloured pill: `<span style="color:#FFF; background-color:#59A14E">  Label  </span>`.
  **The colour must be a literal** — the server validates `background-color` at spec time
  and rejects a `{{ }}` interpolation, so a pill colour can never be conditional.
- `{{ }}` interpolates in a text **`body` only**. An element's `description` is plain
  text and will render the formula literally.
- Column formulas resolve inside `<p>` wrappers, and **without** a sourced element in the
  same container (supersedes the older "must share a container with a sourced element").
- ⚠️ **A bare `<` in an HTML-wrapped body is parsed as a tag open** and kills the whole
  interpolation, which then renders as literal formula text. Write conditions with `>=`
  only (flip the operands) or escape as `&lt;`.
- `{{Now()}}` works for a live timestamp.

## Other

- `style.padding` accepts only `"none"` or omission. `cellSpacing`: `"small"`, not
  `"condensed"`. `borderRadius`: `pill` | `round` | `square`.
- `conditionalFormats[].value` must match the **column's data type**. If the server demands
  a *string* for an obviously numeric column, that is a symptom of a broken column
  reference, not a formatting bug.
- A `chat` element silently drops `name`/`description`.
- **Overlapping sibling elements silently reorder/drop** — keep sibling grid spans disjoint.
- Date `formatString` is **strftime** (`%m/%d/%Y`); `"MM/DD/YYYY"` renders literally.
- **Sigma enforces optimistic concurrency on a round-tripped spec**: PUTting a GET-back
  that is no longer current fails with "The document has changed since it was read."
  Patch scripts must `get_spec()` at runtime, never a saved snapshot.
- **PNG export renders only the ACTIVE tab.** Render a specific element by id, or park
  content in the header panel, to see anything else.
- A PNG taken immediately after a push can show stale/unevaluated content. Wait a few
  seconds; do not debug a formula on one immediate screenshot.
