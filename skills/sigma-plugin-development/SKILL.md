...
### Hosting — localhost is the default

Run your plugin locally and register `http://localhost:3000` (or your dev server port) as the plugin URL in Sigma. Changes hot-reload in the workbook. The default Vite dev server runs on `http://localhost:5173`. For a single-file plugin, `python3 -m http.server <port>` in its directory is enough.

**This is the default for demo and prospect work — use it and move on.** Sigma (HTTPS) loading an HTTP-localhost iframe is permitted as a browser secure-context exception, so it renders correctly on your own machine.

### Production hosting — only when the user asks for it

🚫 **Do not deploy a plugin anywhere external on your own initiative.** No `git push`, no `gh repo create`, no GitHub Pages, jsDelivr, Netlify, Vercel, or S3. A plugin built for a named company is account-identifiable work product, and a public repo or CDN publishes it in a way that can be cached or indexed even after deletion — "not shareable" is an accepted trade-off of localhost, not a problem to route around.

When the user *does* ask for a durable hosted URL: confirm **which** host before creating anything, keep the repo/bucket private unless they say otherwise, and remember the registered plugin `url` is set-once — changing it means a new registration.
